    ...
    <div className="space-y-10">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbItem>
            <BreadcrumbLink asChild>
              <Link to={`/community?topic=${loaderData.post.topic_slug}`}>{loaderData.post.topic}</Link>
            </BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            ...
            <div className="space-y-20 w-full">
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  <span>@{loaderData.post.author}</span>
                  <DotIcon className="size-5" />
                  <span>{DateTime.fromISO(loaderData.post.created_at).toRelative()}</span>
                  <DotIcon className="size-5" />
                  <span>{loaderData.post.replies === 1 ? `${loaderData.post.replies} reply` : `${loaderData.post.replies} replies`}</span>
                </div>
        ...
        <aside className="col-span-2 space-y-5 border rounded-lg p-6 shadow-sm">
          <div className="flex gap-5">
            ...
            <div className="flex flex-col items-start">
              <h4 className="text-lg font-medium">{loaderData.post.author}</h4>
              <Badge variant="secondary" className="capitalize">{loaderData.post.author_role}</Badge>
            </div>
          </div>
